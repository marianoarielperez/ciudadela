...
function setup(member: { email: string | null; emailStatus: string } | null) {
  const receipt = {
    id: 7, number: "2026-00007", pdfPath: "2026/2026-00007.pdf", emailedAt: null, voidedAt: null as Date | null,
...
describe("sendReceiptEmail", () => {
  ...
  // El PDF de un recibo anulado ya no representa nada cobrado.
  it("no manda un recibo anulado", async () => {
    const s = setup({ email: "ana@x.com", emailStatus: "declared" });
    s.receipt.voidedAt = new Date();
    expect(await s.emailer.sendReceiptEmail(7)).toEqual({ sent: false, reason: "error", code: "voided" });
    expect(s.mailer.sendToMember).not.toHaveBeenCalled();
  });

  it("con un id inexistente no tira", async () => {
    const s = setup({ email: "ana@x.com", emailStatus: "declared" });
    s.db.receipt.findUnique.mockResolvedValueOnce(null as never);
