...
```bash
printf "Host github.com-sigev\n  HostName github.com\n  IdentityFile ~/.ssh/sigev_deploy\n  IdentitiesOnly yes\n" >> ~/.ssh/config && git clone git@github.com-sigev:marianoarielperez/ciudadela.git /root/dev/ciudadela
```
...
```bash
cat > /root/dev/ciudadela/.env <<'EOF'
...
EOF
chmod 600 /root/dev/ciudadela/.env
```
...
```bash
cd /root/dev/ciudadela && npm ci && npx prisma migrate deploy && npx prisma db seed && npm run build && pm2 start npm --name sigev -- start && pm2 save
```
...
```bash
chmod +x /root/dev/ciudadela/scripts/backup.sh && /root/dev/ciudadela/scripts/backup.sh && rclone ls gdrive:sigev-backups
```

Expected: 4 archivos `.gpg` (sigev, cbinfra, sir_database, files) locales y en Drive.

```bash
( crontab -l 2>/dev/null; echo "0 4 * * * /root/dev/ciudadela/scripts/backup.sh >> /var/log/sigev-backup.log 2>&1" ) | crontab - && crontab -l
```
