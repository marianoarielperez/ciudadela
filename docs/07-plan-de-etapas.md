...
| 6 | Dos repartos concurrentes → uno gana, el otro lee "cambió mientras"/"ya fue resuelta"; sin huecos | ✅ integración test 2: uno gana, el otro lee "ya fue resuelta"/"cambió"; serie sin huecos (el mutex de proceso serializa; el test de dos instancias asserta sólo el invariante de consistencia, y el orden del lock de fila lo fija el test unitario, verificado por mutación). |
